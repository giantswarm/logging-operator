...
// Grafana-Agent toggle: enable or disable Grafana-Agent in a given Cluster.
...
// ReconcileCreate ensure Grafana-Agent is enabled in the given cluster.
func (r *Reconciler) ReconcileCreate(ctx context.Context, lc loggedcluster.Interface) (ctrl.Result, error) {
	logger := log.FromContext(ctx)
	logger.Info("grafana-agent-toggle create")
	...
	// Get desired configmap to enable grafana-agent.
	...
	logger.Info("grafana-agent-toggle checking", "namespace", desiredConfigMap.GetNamespace(), "name", desiredConfigMap.GetName())
	...
	if err != nil {
		if apimachineryerrors.IsNotFound(err) {
			...
			logger.Info("grafana-agent-toggle not found, creating")
	...
	if needUpdate(currentConfigMap, desiredConfigMap) {
		logger.Info("grafana-agent-toggle updating")
		// Update configmap
		...
		logger.Info("grafana-agent-toggle up to date")
	}

	return ctrl.Result{}, nil
}

// ReconcileDelete ensure Grafana-Agent is disabled for the given cluster.
func (r *Reconciler) ReconcileDelete(ctx context.Context, lc loggedcluster.Interface) (ctrl.Result, error) {
	logger := log.FromContext(ctx)
	logger.Info("grafana-agent-toggle delete")
	...
	// Delete configmap.
	logger.Info("grafana-agent-toggle deleting", "namespace", desiredConfigMap.GetNamespace(), "name", desiredConfigMap.GetName())
	err = r.Client.Delete(ctx, &desiredConfigMap)
	if err != nil {
		if apimachineryerrors.IsNotFound(err) {
			// Do no throw error in case it was not found, as this means
			// it was already deleted.
			logger.Info("grafana-agent-toggle already deleted")
		} else if err != nil {
			return ctrl.Result{}, errors.WithStack(err)
		}
	} else {
		logger.Info("grafana-agent-toggle deleted")
	}
