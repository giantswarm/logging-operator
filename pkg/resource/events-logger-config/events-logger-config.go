...
func generateEventsLoggerConfig(cluster *capi.Cluster, loggingAgent *common.LoggingAgent, tenants []string, includeNamespaces []string, excludeNamespaces []string, installationName string, insecureCA bool, tempoURL string) (v1.ConfigMap, error) {
	var values string
	var err error
	...
	case common.EventsLoggerAlloy:
		values, err = generateAlloyEventsConfig(cluster, includeNamespaces, excludeNamespaces, installationName, insecureCA, tempoURL, tenants)
		if err != nil {
			return v1.ConfigMap{}, err
		}
	default:
